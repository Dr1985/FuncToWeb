...
- **Removed `"reload": False` from `UVICORN_DEFAULTS`** — doubly redundant: `False` is already Uvicorn's default, and `run()` now rejects `reload=True` outright with a `ValueError`.
